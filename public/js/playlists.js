...
function selectPlaylist(index) {
  ...
  document.getElementById("playAllBtn").classList.remove("d-none");
...
document.getElementById("playAllBtn").onclick = () => {
  playQueue = playlists[currentIndex].videos.map(v => v.id.videoId);
  currentPlayIndex = 0;
  playNext();
};
...
function sortSongs(type) {
  if (type === "alpha") {
    playlists[currentIndex].videos.sort((a,b) =>
